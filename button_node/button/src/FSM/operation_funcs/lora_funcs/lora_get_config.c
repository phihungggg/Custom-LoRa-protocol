...
void config_lora_transmit(uint32_t conf_addr){
   static RadioEvents_t RadioEvents;
    ...
    printf(" gio kiem tra thu xem nhu the nao !! \n");
    ...
    for ( int i = 0 ; i < 80 ; ++i)
    {
     ...
     printf(" byte thu %d la %d \n",i,*((volatile uint8_t*)(conf_addr + i))) ;
...
uint8_t address[2];
hihi = get_param_from_flash(PROPERTY_ADDRESS_JUMP[ADDRESS_SET_INDEX],conf_addr,&address);

printf("  address ra xem lay dung chua \n");
for ( int i = 0 ; i < 2 ; ++i )
...
hihi = get_param_from_flash(PROPERTY_ADDRESS_JUMP[AUTHENTICATIONKEY_SET_INDEX],conf_addr,&authentication_key);
