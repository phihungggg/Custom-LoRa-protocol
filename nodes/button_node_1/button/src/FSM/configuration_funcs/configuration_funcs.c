...
States_Config_t authenticcation_key_set(uint16_t lora_buf_size,bool new_data_available,uint8_t *rx_buf,uint8_t *config_data,uint8_t *config_data_current_index ){


    States_Config_t key_result = AUTHENTICATION_KEY_SET;
...
if ( new_data_available==true)
{
if ( lora_buf_size-2==19)
{
    printf(" authentication right format !! \n");
    ...
    printf(" check rx buf of authentication key  \n");


    for ( int i =1 ; i <=lora_buf_size-1;++i )
    {
    ...
    num_bytes = parse_strict_hex_bytes_ver2(rx_buf+1,key_bytes,4);

    printf(" num bytes la %d \n",num_bytes);
    if( num_bytes ==4 )
    {
      ...
      //  key_result = TX_POWER_SET;
        config_data[(*(config_data_current_index))++]= num_bytes;
        for ( int i =0 ;i<num_bytes;++i)
        {
            config_data [i+(*(config_data_current_index))] =key_bytes[i];
        }
        (*(config_data_current_index)) =(*(config_data_current_index)) +num_bytes;
    
        key_result = NODE_ADDRESS_SET;
...
States_Config_t address_key_set(uint16_t lora_buf_size,bool new_data_available,uint8_t *rx_buf,uint8_t *config_data,uint8_t *config_data_current_index ){

States_Config_t key_result = NODE_ADDRESS_SET;
...
if ( new_data_available== true)
{
    if (lora_buf_size-2==9)
    {
       ...
       uint8_t key_bytes[2] ;
       int num_bytes;
       num_bytes = parse_strict_hex_bytes_ver2(rx_buf+1,key_bytes,2);
       if( num_bytes ==2 )
       {
           for ( int i =0;i<num_bytes;++i)
           {
         ...
         //  key_result = TX_POWER_SET;
           ...
           key_result = ADD_CONFIG_TO_FLASH_MEMORY;
